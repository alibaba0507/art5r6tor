...
//error_log("// WE ARE HERE ...............");
	...
	$url = "http://answers.yahoo.com/search/search_result?p=".$_GET['keyword']."&submit-go=Search+Y!+Answers".$end;
//error_log("// WE ARE HERE ...............");
...
if (isset($_GET['key']) && ($key_index = array_search($_GET['key'], $options->api_keys)) !== false) {
	...
	$redirect = 'http://'.htmlspecialchars($host.$path).'/yahooanswers.php?keyword='.urlencode($_GET['keyword']).$end;
...
if (!$html_only) {
	...
	//$feed->get_title();
	debug("// ***************** Feed". var_dump($result)." **********");
	
	debug(var_dump($feed->data));
